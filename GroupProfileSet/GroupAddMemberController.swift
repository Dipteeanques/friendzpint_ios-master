...
class GroupAddMemberController: UIViewController,UISearchBarDelegate {
    ...
    func getSearch() {
        ...
        wc.callSimplewebservice(url: GET_USERS, parameters: parameters, headers: headers, fromView: self.view, isLoading: false) { (sucess, response: MemberSearchResponsModel?) in
            if sucess {
...
extension GroupAddMemberController: UITableViewDelegate,UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return arrResults.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tblMemberFriends.dequeueReusableCell(withIdentifier: "cell", for: indexPath)as! GroupAddMemberCell
        ...
        cell.btnJoin.addTarget(self, action: #selector(GroupAddMemberController.btnJoinAction), for: UIControl.Event.touchUpInside)
    ...
    @objc func btnJoinAction(_ sender : UIButton) {
        if let indexPath = self.tblMemberFriends.indexPathForView(sender) {
            let cell = tblMemberFriends.cellForRow(at: indexPath) as! GroupAddMemberCell
            ...
            wc.callSimplewebservice(url: ADD_MEMBERGROUP, parameters: parameters, headers: headers, fromView: self.view, isLoading: true) { (sucess, response: JoinandUnjoinedResponse?) in
                if sucess {
