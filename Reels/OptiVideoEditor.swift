...
class OptiVideoEditor: NSObject {
    ...
    func addfiltertoVideo(strfiltername : String, strUrl : URL, success: @escaping ((URL) -> Void), failure: @escaping ((String?) -> Void)) {
        ...
        //Create Directory path for SavecachesDirectory
        let documentDirectory = FileManager.default.urls(for: .cachesDirectory, in: .userDomainMask)[0]
        let timestamp = NSDate().timeIntervalSince1970
        var outputURL = documentDirectory.appendingPathComponent("EffectVideo\(timestamp)")
        do {
