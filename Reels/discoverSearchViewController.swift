...
extension discoverSearchViewController: UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if tableView == userTV{
            ...
            userCell.userImg.sd_setIndicatorStyle(.gray)
        ...
        else
        {
            ...
            soundCell.soundImg.sd_setIndicatorStyle(.gray)
...
//                navigationController?.pushViewController(vc, animated: true)
                let obj1 = self.storyboard?.instantiateViewController(withIdentifier: "navigationLoaderpageredirection")as! navigationLoaderpageredirection
                obj1.strUser = obj.username
//                    self.navigationController?.pushViewController(obj, animated: true)
                obj1.modalPresentationStyle = .overFullScreen
                self.present(obj1, animated: false, completion: nil)
            }
...
//    MARK:- SELECT SOUND ACTION
    @objc func btnSelectAction(_ sender : UIButton) {
        ...
        let uid = UserDefaults.standard.string(forKey: "TimeLine_id")
...
//    MARK:- ADD FAV SOUND FUNC
    func addFavSong(soundID:String,btnFav:UIButton){
        ...
        ApiHandler.sharedInstance.addSoundFavourite(user_id: UserDefaults.standard.string(forKey: "TimeLine_id")!, sound_id: soundID) { (isSuccess, response) in
            if isSuccess{
...
//            let vidImg = baseUrl+vidObj.userProfile_pic
            ...
            vidCell.vidImg.sd_setIndicatorStyle(.gray)
...
//            vidCell.vidImg.image = imageURL
            ...
            vidCell.userImg.sd_setIndicatorStyle(.gray)
        ...
        if indexPath.row == 0 {
            if self.objArr[indexPath.row]["isSelected"] == "false"{
                ...
                cell.titleLbl.text = objArr[indexPath.row]["title"]
                cell.titleLbl.textColor = #colorLiteral(red: 0.137254902, green: 0.4117647059, blue: 1, alpha: 1)
            }
        }
        if indexPath.row == 1 {
            if self.objArr[indexPath.row]["isSelected"] == "false"{
