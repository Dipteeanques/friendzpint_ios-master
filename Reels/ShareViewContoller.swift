...
class ShareViewContoller: UIViewController,UICollectionViewDataSource,UICollectionViewDelegate, UICollectionViewDelegateFlowLayout,MFMessageComposeViewControllerDelegate {
    ...
    let arrShare = ["Copy link","Save","Share to...","Delete","Not interested"]
...
//        shareUrl = BASE_URL+"?"+randomString(length: 3)+videoID+randomString(length: 3)
        shareUrl = currentVideoUrl
        objToShare.append(shareUrl)
        

        curveView.clipsToBounds = true
        curveView.layer.cornerRadius = 25
        curveView.layer.maskedCorners = [.layerMaxXMaxYCorner, .layerMinXMaxYCorner]
        
        curveView.clipsToBounds = true
        curveView.layer.cornerRadius = 25
        curveView.layer.maskedCorners = [.layerMaxXMinYCorner, .layerMinXMinYCorner]
        
        NotificationCenter.default.addObserver(self, selector: #selector(dismissVCnoti(notification:)), name: Notification.Name("dismissVCnoti"), object: nil)
    }
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if collectionView == share1CV{
            ...
            case "saveVideo":
                print("saveVideo Tap")
                _ = URL.init(fileURLWithPath: (AppUtility?.detectURL(ipString: currentVideoUrl))!)
...
//                    self.alertModule(title:"Tictic", msg: "Please login from Profile to Report")
                ...
                if(UserDefaults.standard.string(forKey: "TimeLine_id") == "" || UserDefaults.standard.string(forKey: "TimeLine_id") == nil){
        ...
        requestAuthorization {
            ...
            DispatchQueue.global(qos: .background).async {
                if let url = URL(string: vidUrlString),
                    ...
                    let filePath="\(documentsPath)/\(timestamp).mp4"//tempFile
                    print("filePath: ",filePath)
                    DispatchQueue.main.async {
                        urlData.write(toFile: filePath, atomically: true)
                        PHPhotoLibrary.shared().performChanges({
...
extension ShareViewContoller: UITableViewDelegate, UITableViewDataSource{
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return arrShare.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as! ShareCell
        cell.lblName.text = arrShare[indexPath.row]
        return cell
    }
