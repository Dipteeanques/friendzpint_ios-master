...
class DetailsPostandComment: UIViewController,UIPopoverPresentationControllerDelegate,TTTAttributedLabelDelegate,PSHTMLViewDelegate {//UIWebViewDelegate
...
//        getfirstTime()
       ...
       // navigationController?.setStatusBar(backgroundColor: .black)
        ...
        imgProfileComment.layer.cornerRadius = 15
        imgProfileComment.clipsToBounds = true
    }
        ...
        wc.callSimplewebservice(url: PARENT_TIMELINE_COMMENTS, parameters: parameters, headers: headers, fromView: self.view, isLoading: false) { (sucess, response: ParentCommentResponsModel?) in
            if sucess {
                ...
                if sucessMy! {
                    ...
                    print( self.arrParentComment)
                  //  self.tblFeed.beginUpdates()
                    self.arrCommented[self.indexPath.section].replyCommects = response!.data.data
                    self.tblFeed.reloadData()
//                    self.tblFeed.insertRows(at: [IndexPath(row: self.arrParentComment.count, section: 1)], with: .fade)
//                    self.tblFeed.endUpdates()
    ...
    func sendComment() {
        if replay == "replay" {
            if txtCommentBOX.text!.trimmingCharacters(in: .whitespaces).isEmpty {
                print("space")
            }
            else {
                let parameters = ["post_id": postDetail_id,
                ...
                wc.callSimplewebservice(url: POSTCOMMENT, parameters: parameters, headers: headers, fromView: self.view, isLoading: true) { (sucess, response: ParentCommentListCommentResponseModel?) in
                    if sucess {
            ...
            if txtCommentBOX.text!.trimmingCharacters(in: .whitespaces).isEmpty {
                ...
                wc.callSimplewebservice(url: POSTCOMMENT, parameters: parameters, headers: headers, fromView: self.view, isLoading: true) { (sucess, response: CreateCommentResponseModel?) in
                    if sucess {
                        let sucessRes = response?.success
                        if sucessRes! {
                            ...
                            self.tblFeed.reloadData()
                            self.txtCommentBOX.text = nil
                            NotificationCenter.default.post(name: NSNotification.Name(rawValue: "CommentCount"), object: self.postDetail_id)
                        }
                    }
                }
            }
...
extension DetailsPostandComment: UITableViewDelegate,UITableViewDataSource,UIScrollViewDelegate {
    func numberOfSections(in tableView: UITableView) -> Int {
        return arrFeed.count + arrCommented.count
    }
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        case 0:
            return 1
        default:
            if let arrchaildComment = arrCommented[section - 1].replyCommects {
...
//            }
        default:
                ...
                let strComment = arrCommented[indexPath.section - 1].replyCommects![indexPath.row].description
                cell.lblReply.text = strComment
                cell.lblreplyname.text = arrCommented[indexPath.section - 1].replyCommects![indexPath.row].user_name
                let strImage = arrCommented[indexPath.section - 1].replyCommects![indexPath.row].user_avatar
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        case 0:
            return UITableViewCell()
        default:
            let cell = tblFeed.dequeueReusableCell(withIdentifier: "commentCell")as! detailsCommentCell
            cell.arrCommentSet = arrCommented[section - 1]
            cell.btnReply.addTarget(self, action: #selector(DetailsPostandComment.btnreplayAction), for: UIControl.Event.touchUpInside)
            cell.btnReply.isHidden = false
            cell.btnLike.addTarget(self, action: #selector(DetailsPostandComment.btnLikeAction), for: UIControl.Event.touchUpInside)
            cell.btnSeemore.addTarget(self, action: #selector(DetailsPostandComment.btnseemoreAction), for: UIControl.Event.touchUpInside)
    ...
    func reloadData(){
        ...
        DispatchQueue.main.async {
            ...
            self.tblFeed.scrollToRow(at: indexPath as IndexPath, at: .bottom, animated: false)
...
//            self.navigationController?.pushViewController(obj, animated: true)
    ...
    @objc func btnLikeAction(_ sender: UIButton) {
        ...
        wc.callSimplewebservice(url: LIKECOMMENT, parameters: parameters, headers: headers, fromView: self.view, isLoading: false) { (sucess, response: commentLikeResponsModel?) in
            if sucess {
                if response!.liked {
                    self.arrCommented = self.arrCommented.map{
                        var mutableBook = $0
                        if $0.id == self.parent_ID {
...
extension DetailsPostandComment: UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout {
...
extension DetailsPostandComment: SquareFlowLayoutDelegate {
