...
class EventMemberController: UIViewController,UISearchBarDelegate {
    ...
    func getSearch() {
        let page_id = loggdenUser.value(forKeyPath: EVENTID)as! Int
        let trimmed = strSearchTxt.trimmingCharacters(in: .whitespacesAndNewlines)
        let parameters = ["event_id": page_id,
        ...
        wc.callSimplewebservice(url: GET_MEMBERS_INVITE, parameters: parameters, headers: headers, fromView: self.view, isLoading: false) { (sucess, response: pageSearchAddmemberrespons?) in
            if sucess {
...
extension EventMemberController: UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if user_status == 0 {
            cell.btnJoin.setTitle("Invite", for: .normal)
        }else {
            cell.btnJoin.setTitle("Invited", for: .normal)
        }
        return cell
    }
...
//            self.navigationController?.pushViewController(obj, animated: true)
    ...
    @objc func btnJoinAction(_ sender : UIButton) {
        if let indexPath = self.tblMemberFriends.indexPathForView(sender) {
            ...
            wc.callSimplewebservice(url: ADD_EVENT_MEMBERS, parameters: parameters, headers: headers, fromView: self.view, isLoading: true) { (sucess, response: JoinandUnjoinedResponse?) in
                if sucess {
