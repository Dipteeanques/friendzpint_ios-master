...
class PagelikedViewController: UIViewController {
        ...
        wc.callSimplewebservice(url: MY_LIKED_PAGES, parameters: parameters, headers: headers, fromView: self.view, isLoading: true) { (sucess, response: UserPageLikeGroupResponseModel?) in
            if sucess {
                ...
                if sucessMy! {
                    let data = response?.data
                    let arr_dict  = data?.data
                    print("arr_dict_page:",arr_dict)
                    for i in 0..<arr_dict!.count
                    {
                        self.arrLiked.insert(arr_dict![i], at: 0)
...
extension PagelikedViewController: UITableViewDelegate,UITableViewDataSource,UIScrollViewDelegate {
