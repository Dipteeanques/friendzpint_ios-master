...
//        UIApplication.shared.statusBarStyle = .lightContent
    ...
    func firstTime(){
        ...
        if strUrlType == "FTimeline"{
            ...
            strUrl = SHOWFEED
            parameters = ["hashtag": hashtagpost,
    ...
    func GetSaveList(){
        ...
        print("headers: ",headers)
        print("BROWSE: ",BROWSE)
        wc.callSimplewebservice(url: strUrl, parameters: parameters, headers: headers, fromView: self.view, isLoading: false) { (sucess, response: MyTimelineResponseModel?) in
            //            print("response:",response.re)
            if sucess {
                let sucessMy = response?.success
                if sucessMy! {
                    ...
                    if self.arrFeed1.count == 0{
                        self.arrFeed1 = response!.data.data!
                        //                        self.mainTableView.scrollToTop()
                        self.refreshControl.endRefreshing()
                        self.spinner.stopAnimating()
                        self.mainTableView.reloadData()
